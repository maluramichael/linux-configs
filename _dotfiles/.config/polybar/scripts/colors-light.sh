...
if [[ $1 = "-amber" ]]; then
  # Replacing colors
  sed -i --follow-symlinks -e 's/bg = .*/bg = #f5f5f5/g' $PDIR/colors.ini
  sed -i --follow-symlinks -e 's/fg = .*/fg = #252525/g' $PDIR/colors.ini
  ...
  sed -i --follow-symlinks -e 's/bspc config normal_border_color ".*"/bspc config normal_border_color "#ffffff"/g' $BSPWMRC
  # Restarting polybar
  $LAUNCH &
  bspc wm -r
